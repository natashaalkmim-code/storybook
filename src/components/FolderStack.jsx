...
export default function FolderStack() {
  ...
  useEffect(() => {
    ...
    // OPEN --------------------------------------------------------------
    // Mirrors CLOSE, but as ONE continuous motion instead of two tweens
    // handed off to each other — that handoff was the "travado" moment.
    // Y takes a detour through a lift point on its way to centre (rising
    // out from behind its own divider) while x/width/height grow the whole
    // time in the same breath; the z-index bump to the front happens
    // silently mid-flow, once it's clear of the stack, without breaking
    // the motion's curve. The flat colour panel, not the tilted artwork,
    // is what moves, so it's a clean right angle throughout; the divider
    // it came from, and every other pair, never move.
    if (from < 0 && to >= 0) {
      ...
      const liftY = m.sheet.y - liftOffset;

      tl.to(selectedSheet, {
        keyframes: {
          '22%': { y: liftY, ease: 'power2.out' },
          '100%': {
            x: 0,
            y: 0,
            width: viewport.width,
      ...
      // Rises above the whole stack here, silently — nothing about the
      // motion itself changes at this instant, only what's in front of what.
      tl.set(selectedSheet, { zIndex: OVERLAY_SHEET_LAYER }, duration * 0.26);
